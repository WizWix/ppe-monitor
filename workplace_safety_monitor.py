    ...
    def __init__(self, weights: Optional[str] = None, conf: float = 0.45):
        ...
        if self.use_ultra:
            try:
                ...
                dummy = [np.zeros((640, 640, 3), dtype=np.uint8)] * 8
...
class PPEUltralytics:
    def __init__(self, weights: str, conf_th: float, nms_iou: float, class_map: Dict[int, str]):
        ...
        _ = self.model([np.zeros((640, 640, 3), dtype=np.uint8)] * 8, conf=self.conf_th, iou=self.nms_iou, verbose=False)
...
class IoUTracker:
    def __init__(self, iou_th: float = 0.3, max_age: int = 20, min_hits: int = 1):
        self.iou_th = iou_th
        self.max_age = max_age
        self.min_hits = min_hits  # track must be matched ≥ min_hits times before being reported
    ...
    def update(self, detections: List[Box], frame_width: int = None, frame_height: int = None) -> Dict[int, Box]:
        ...
        # Return only mature tracks (seen ≥ min_hits times)
        return {tid: tr.box for tid, tr in self.tracks.items() if tr.hits >= self.min_hits}
